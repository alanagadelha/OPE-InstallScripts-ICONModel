...
&master_model_nml
 ...
 model_namelist_filename     =	"NAMELIST_ICONMET"	! file name containing the model namelists
...
&time_nml
 ...
 end_datetime_string         = "$HSTOP"        		! end date and time of the simulation. Example date: 2001-01-01T01:00:00Z
/
EOF
...
&extpar_nml
 itopo                       =                          1         ! topography (0:analytical; 1:ext. data read from file).
 n_iter_smooth_topo          =                          1         ! iterations of topography smoother
 itype_lwemiss		     =				2	  ! Type of data used for longwave surface emissivity (2:use mon clim  from satellite measurements)
 heightdiff_threshold        =                       3000.        ! height difference between neighb. grid points
 hgtdiff_max_smooth_topo     =                        750.        ! see Namelist doc
 itype_vegetation_cycle      =                          1         ! 1: annual cycle of LAI solely based on NDVI climatology. ICON-EU uses 2 for IAU.
...
! Many fields changed for this nml in ICON-EU does not apply to us, since we don't use IAU.
&initicon_nml
 init_mode                   =                          7         ! start from DWD fg with subsequent vertical remapping (ICON-EU uses IAU =5)
 lread_ana                   =                     .false.        ! no analysis data will be read (true only for IAU and DWDANA)
 ...
 ltile_coldstart             =                      .TRUE.        ! coldstart for surface tiles (ICON-EU is FALSE, for IAU)
...
/
! If support_baryctr_intp=true, model gets stuck at setup_barycentric_intp_lonlat_repartition 
...
&nonhydrostatic_nml
 ...
 damp_height                 =                      18000.0       ! height at which Rayleigh damping of vertical wind starts. Daniel suggested 18000.0.
 rayleigh_coeff              =                          1.0       ! Rayleigh damping coefficient. Doubt.
 ...
 exner_expol                 = 0.59999999999999998	          ! temporal extrapolation of Exner function
...
&nwp_phy_nml
 ...
 dt_conv                     =                          180.0     ! time step for convection in s (domain specific)
 dt_rad                      =                          1440.0    ! time step for radiation in s
 dt_sso                      =                          360.0     ! time step for SSO parameterization
 dt_gwd                      =                         	720.0     ! time step for gravity wave drag parameterization
 icpl_aero_gscp              =                          1         ! coupling between autoconversion and Tegen aerosol climatology
 efdt_min_raylfric           =                          3600.0    ! minimum e-folding time of Rayleigh friction
 ...
 lupatmo_phy                 =                      .FALSE.	  ! Switch for upper-atmosphere physics.
/

! nwp_tuning_nml: additional tuning parameters ---------------------------------- Doubt, should we use this part too?
&nwp_tuning_nml
...
&radiation_nml
 ...
 direct_albedo_water         =                          3         ! Direct beam surface albedo over water (ocean or lake).
 albedo_whitecap	     =				1	  ! Ocean albedo increase by foam from breaking waves (whitecaps).
 islope_rad                  =                          0         ! Slope correction for surface radiation
 vmr_co2                     =                    3.899e-004	  ! Volume mixing ratio of CO2 (ecRad specific)
 vmr_ch4                     =                    1.799e-006	  ! Volume mixing ratio of CH4 (ecRad specific)
 vmr_n2o                     =                    3.220e-007	  ! Volume mixing ratio of N2O (ecRad specific)
...
&sleve_nml
 ...
 max_lay_thckn		     = 400.0
 htop_thcknlimit	     = 14000.0
 top_height                  =                      30000.0       ! height of model top. Daniel suggested 30000.0.
 stretch_fac                 = 0.90000000000000002		  ! stretching factor to vary distribution of model levels
...
&transport_nml
 ihadv_tracer                =          200*2         ! tracer specific method to compute horizontal advection
 ivadv_tracer                =          200*3         ! tracer specific method to compute vertical advection
 itype_hlimit                =          200*4         ! type of limiter for horizontal transport
 llsq_svd                    =          .TRUE.        ! use SV decomposition for least squares design matrix
! ctracer_list		= '12345'		      ! Doubt, couldn't find this variable in the ref.
/
...
&turbdiff_nml
 ...
 tkhmin                      = 0.59999999999999998	          ! scaling factor for minimum vertical diffusion coefficient
 ...
 alpha1			     = 0.1250				  ! Scaling parameter for molecular roughness of ocean waves
 tkhmin_strat		     = 1.0				  ! Scaling factor for stratospheric min vert diff coeff for heat & moist >17.5km (tropics, >22.5km)
 tur_len                     =                        500.0       ! asymptotic maximal turbulent distance
 rat_sea                     =                          0.80	  ! controls laminar resistance for sea surface
...
#/home/devicon/instalacao-intel2019.5/libraries/bin/mpiexec -f /home/opicon/operacional/data/const/icon_model_nodes.txt -n 1152 $MODEL
/home/devicon/instalacao-intel2019.5/libraries/bin/mpiexec -f /home/opicon/operacional/data/const/icon_model_nodes.txt -n 672 $MODEL
